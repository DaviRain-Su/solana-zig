...
## Goals
- 完成 11 个扩展 RPC 方法的 mock 测试覆盖与 Devnet E2E 验证
- 完成 Websocket 客户端生产级硬化：7 种订阅类型、重连/去重/resubscribe、可观测性指标
- 实现 RPC 统一重试策略（exponential backoff + max retries + rate limit 感知）
- 扩展 benchmark 基线：新增 RPC 响应解析与 Websocket 消息编解码 benchmark
- 保证 Phase 1 现有测试最终回归通过，允许必要的重构
...
**Acceptance Criteria:**
- [ ] 支持按签名查询交易详情
- [ ] 返回 slot、blockTime、meta（fee、logMessages、err 等）
- [ ] 支持 commitment 与 maxSupportedTransactionVersion 参数
...
**Acceptance Criteria:**
- [ ] 返回签名列表（含 slot、blockTime、err、memo）
- [ ] 支持 before/until/limit 分页参数
...
**Acceptance Criteria:**
- [ ] 支持批量签名输入
- [ ] 返回每个签名的 slot、confirmations、err、confirmationStatus
- [ ] 支持 searchTransactionHistory 参数
- [ ] Mock 测试覆盖成功、部分不存在、错误路径
- [ ] Devnet E2E 验证：发送交易后轮询状态

### US-004: getSlot / getEpochInfo RPC 方法
As a Zig 开发者, I want 调用 getSlot 和 getEpochInfo so that 我可以获取网络时隙与纪元信息。

**Acceptance Criteria:**
- [ ] getSlot 返回当前 slot（u64）
- [ ] getEpochInfo 返回 epoch、slotIndex、slotsInEpoch、absoluteSlot、blockHeight、transactionCount
- [ ] 支持 commitment 参数
- [ ] Mock 测试覆盖成功和错误路径
...
**Acceptance Criteria:**
- [ ] 返回 Token 账户列表（pubkey、accountInfo）
- [ ] 支持 programId 和 mint 两种过滤条件
...
**Acceptance Criteria:**
- [ ] getTokenAccountBalance 返回 amount、decimals、uiAmountString
- [ ] getTokenSupply 返回相同结构
...
**Acceptance Criteria:**
- [ ] 实现 exponential backoff 重试机制
- [ ] 支持配置 maxRetries、baseDelayMs、maxDelayMs
- [ ] 对 HTTP 429（rate limit）和 transient 网络错误触发重试
- [ ] 对不可恢复错误（如 400 Bad Request）直接失败
- [ ] Mock 测试覆盖重试成功、重试耗尽、非重试错误路径

### US-011: Websocket 订阅类型扩展
As a Zig 开发者, I want Websocket 客户端支持完整订阅类型 so that 我可以实时监听链上事件。

**Acceptance Criteria:**
- [ ] 支持 accountSubscribe / accountUnsubscribe
- [ ] 支持 programSubscribe / programUnsubscribe
- [ ] 支持 signatureSubscribe / signatureUnsubscribe
- [ ] 支持 slotSubscribe / slotUnsubscribe
- [ ] 支持 rootSubscribe / rootUnsubscribe
- [ ] 支持 logsSubscribe / logsUnsubscribe
- [ ] 支持 blockSubscribe / blockUnsubscribe
- [ ] 每种订阅类型有独立的回调类型与反序列化逻辑
...
**Acceptance Criteria:**
- [ ] 暴露连接状态（connected / disconnected / reconnecting）
- [ ] 暴露活跃订阅数量
- [ ] 暴露已发送/已接收消息计数
- [ ] 暴露重连次数计数
- [ ] 提供查询接口或回调供外部集成
...
- FR-01: 支持 11 个扩展 RPC 方法（getTransaction、getSignaturesForAddress、getSignatureStatuses、getSlot、getEpochInfo、getMinimumBalanceForRentExemption、requestAirdrop、getAddressLookupTable、getTokenAccountsByOwner、getTokenAccountBalance、getTokenSupply）并保留结构化错误
- FR-02: RPC 客户端必须实现统一重试策略，支持 exponential backoff、max retries、rate limit 感知
- FR-03: Websocket 客户端必须支持 7 种订阅类型（account、program、signature、slot、root、logs、block）
- FR-04: Websocket 客户端断线后必须自动重连并恢复所有活跃订阅
- FR-05: Websocket 客户端必须暴露连接状态、订阅数量、消息计数、重连次数等运行时指标
...
- WebSocket TLS (wss) 强制支持 — 视 std.crypto.tls 或外部依赖可行性而定，不阻塞交付
- Durable Nonce 工作流 — Phase 3
- Compute Budget / Priority Fees 接口层 — Phase 3
- Token Program 高级抽象（如 ATA create、close）— Phase 3
- 异步 I/O（继续使用阻塞模式）
- 自定义序列化框架（继续使用手写 bincode 兼容层）

## Technical Considerations
- **基线锁定**: solana-sdk 4.0.1 / solana-client 3.1.12，所有行为与字节布局对齐此版本
- **模块依赖方向**: core → tx → rpc → ws，禁止反向依赖
- **Zig 版本**: 0.16.0
- **HTTP**: std.http.Client 阻塞 I/O
- **Websocket**: 基于 std.net 或现有 ws_client.zig 架构扩展
- **重试策略**: 建议在 RpcClient 层统一封装，避免每个方法重复实现
