...
>
> Note: current Zig mainline has already moved to typed `AccountInfo` / `SimulateTransactionResult`. This v0.1.0 package should therefore be read as a **minimal compatibility shim subset**, not full RPC parity with the latest Zig public API.
...
| **AC-03** | 当前 v0.1 仍以兼容 shim 方式保留原始 JSON | `Connection.simulateTransaction()` → `RpcResult<OwnedJson>` | `connection.test.ts` "AC-03: simulateTransaction preserves raw JSON" (2 tests) | 返回 `OwnedJson { value, raw }` 保留完整 JSON；**注意**：这还不是与最新 Zig `SimulateTransactionResult` 完全对齐的 typed parity |
