...
- Product roadmap 范围变化：必须同步更新 `docs/01/04/05/10/15`。
- 当前 Milestone 调整：必须同步更新 `docs/04/06/07/10/15`。
- 风险变化：必须同步更新 `docs/07`（审查报告）。
- 兼容变化：必须同步更新 `docs/08`（本文件）。
- 子规格进入实现阶段：必须同步更新 `docs/04/05/10`。
- Phase 1 收口信号变化：必须同步更新 `docs/11/12/13/14/15` 中的相应条目。
