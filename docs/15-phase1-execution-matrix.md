...
| 能力项 | 当前状态 | 对应任务 | 当前 blocker | 收口证据 | 证据落点 | Closeout 条件 |
|---|---|---|---|---|---|---|
| `rpc.Batch B methods` | in-review | `#27`, `#55` | **主干化已完成，但方法级 integration / exception 仍待 `#56` 环境补跑。** `#55` 已在 `6d5f1be` 将 `getEpochInfo/getMinimumBalanceForRentExemption/requestAirdrop/getAddressLookupTable` 合并到 `main`，并以隔离 worktree 固化 canonical 三件套（clean, `148/148 tests passed`）。当前 blocker 仅剩：`SOLANA_RPC_URL` / `SURFPOOL_RPC_URL` 未提供，导致 `public devnet` / `local-live` live 证据仍为 environment-gated pending | Batch B 4 方法 typed parse + `happy/rpc_error/malformed`；landing 后 canonical 三件套（`6d5f1be`）；后续仍需按 Batch 7 机械规则补齐：`getEpochInfo/getMinimumBalanceForRentExemption` public devnet、`requestAirdrop` 至少一侧成功、`getAddressLookupTable` RPC error evidence / exception | `src/solana/rpc/client.zig` + `src/solana/rpc/types.zig` + `docs/06` + `docs/10` + 本矩阵 | 由 `#56` 在环境就绪后补跑 live/integration；随后按 `docs/29` 完成 Batch 7 exception / closeout 复核 |
