...
pub const MemoParams = struct {
    pub const SignerMode = union(enum) {
        none,
        single: pubkey_mod.Pubkey,
    };
...
/// Accounts (signer_mode):
/// - none: no account metas
/// - single: [0] signer (readonly signer)
pub fn buildMemoInstruction(
    ...
    const account_len: usize = switch (params.signer_mode) {
        .none => 0,
        .single => 1,
    };
    const accounts = try allocator.alloc(instruction_mod.AccountMeta, account_len);
    errdefer allocator.free(accounts);
    switch (params.signer_mode) {
        .none => {},
        .single => |signer| {
            accounts[0] = .{
                .pubkey = signer,
...
test "memo instruction supports no-signer mode" {
    const allocator = std.testing.allocator;
    const memo_text = "memo-without-signer";
...
test "memo builder compile-sign evidence" {
    ...
    const memo_ix = try buildMemoInstruction(allocator, .{
        .signer_mode = .{ .single = payer.pubkey() },
...
test "memo builder compile-sign evidence without signer-mode account" {
    const allocator = std.testing.allocator;

    const payer = try keypair_mod.Keypair.fromSeed([_]u8{0x7B} ** 32);
    const memo_ix = try buildMemoInstruction(allocator, .{
        .signer_mode = .none,
        .memo = "memo-no-signer-compile-sign",
    });
    defer allocator.free(memo_ix.data);
    defer allocator.free(memo_ix.accounts);

    const ixs = [_]instruction_mod.Instruction{memo_ix};
    const recent_blockhash = hash_mod.Hash.init([_]u8{0x9B} ** 32);
    const msg = try message_mod.Message.compileLegacy(allocator, payer.pubkey(), &ixs, recent_blockhash);
